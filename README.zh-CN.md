...
- **移动端自适应**：手机/平板访问自动切换为侧栏抽屉 + 源/结果双面板切换器，按钮触控目标 ≥40px，复制按钮兼容 iOS Safari。
- **批量解析队列**：一次选中多个文件（或拖入整个文件夹）自动归入侧栏文件夹并串行解析；队列条支持暂停、跳过当前、重试失败、停止清空；跨刷新持久化、重启后默认暂停可手动继续。
- **批量操作已有任务**：任务列表支持多选模式（搜索框旁对勾按钮），可一次性批量解析、移动到文件夹或删除已有任务。
- **延迟加载源文件**：打开页面时不会自动渲染上次选中的 PDF，避免大文件卡住；显示轻量占位卡，需要时点"加载源文件"按钮再渲染。
