...
PaddleOCR Local is a lightweight Web frontend for PaddleOCR-VL, PP-OCRv6, MinerU, and optional Unlimited-OCR. The frontend handles file upload, queueing, preview, model switching, and download, while the FastAPI backend serves static files, converts Office files to PDF, and proxies requests. OCR inference runs in separate model services. The NVIDIA path uses Docker Compose to manage PaddleOCR / MinerU / Unlimited-OCR services, and the macOS Apple Silicon path uses local PaddleX/MLX services.

**Four supported core models + one optional experimental model:**
...
| Unlimited-OCR (optional) | Long-document parsing | Experimental integration with Transformers / SGLang backends and streaming output |
